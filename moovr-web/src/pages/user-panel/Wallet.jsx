  ...
  try {
    ...
    if (selectedMethod === "Paystack") {
      const response = await axios.post(
        `${BaseURL}/wallet/paystack/initialize`,
        {
    ...
    // Handle Stripe-based methods (Stripe, Google Pay, Apple Pay, Mastercard)
    const response = await axios.post(
    ...
    toast.error(
      error.response?.data?.details || error.message || "Payment failed. Please try again."
    );
  }
};
          ...
          <div className="card-gradient p-6 rounded-lg shadow-md w-full max-w-[370px] h-[180px] relative">
            <div className="flex justify-end items-center">
              {JSON.parse(localStorage.getItem("user"))?.profilePicture ? (
                ...
                <FaUserCircle className="h-8 w-8 text-white opacity-80" />
              )}
            </div>
